...
if($role=='Doctor'){
	$check="SELECT doctor_id FROM doctors as d inner join user as u on d.user_id=u.id WHERE u.email='$email' ;";
   $result=mysqli_query($connection , $check) or die("failed to insert query.");
if($result){
...
$_SESSION['isdoctor']=true;
	...
	$check="SELECT patients_id FROM patients WHERE email='$email' ;";
   $result=mysqli_query($connection , $check) or die("failed to insert query.");
if($result){
if(mysqli_num_rows($result) == 1){
$row=mysqli_fetch_assoc($result);
$patients_id=$row['patients_id'];
	...
	$_SESSION['patients_id']=$patients_id;
		echo "<script>alert('Successfully logged in.')
    ...
    echo "<script>alert('Invalid Credentials.')</script>;";
     ...
     echo "<script>alert('You are not a user. Please register an account.')
