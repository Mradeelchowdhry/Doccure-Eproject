...
<?php 
$read= "SELECT * FROM `patients` WHERE patients_id=$pat_id";
$leAo=mysqli_query($connection , $read);
if($result){
	if(mysqli_num_rows($leAo) == 1){
	$row=mysqli_fetch_assoc($leAo);
	$pname=$row['name'];
	$address=$row['address'];
	$phone=$row['phone'];
	$email=$row['email'];
...
?>
													...
													<a href=".." class="booking-doc-img">
														<img src="<?=$pat_image?>" alt="User Image">
													</a>
													<div class="profile-det-info">
														<h3><?php $pname?></h3>
														
														<div class="patient-details">
															<h5><b>Patient ID :</b> <?php $pat_id;?></h5>
															<h5 class="mb-0"><i class="fas fa-map-marker-alt"></i>  <?php $address?> ,USA</h5>
														</div>
													</div>
												</div>
											</div>
											<div class="patient-info">
												<ul>
													<li>Phone<span>  <?php $phone?> </span></li>
